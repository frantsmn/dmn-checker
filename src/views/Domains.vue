		...
		<div class="card border-secondary rounded-2">
			<div class="row card-body">
                ...
                <label class="custom-control-label" for="sort-by-donors">по количеству доноров</label>
						...
						<button class="btn btn-sm btn-outline-info" @click="sort = null">
							Сбросить
						</button>
					</div>
				</div>
						...
						<div class="form-group mb-2">
							<div class="custom-control custom-switch">
								...
								<label class="custom-control-label" for="hideErrors">
									Скрывать результаты с ошибками
								</label>
							</div>
						</div>
					</div>
...
export default {
	...
	methods: {
		...
		async onSubmit() {
			...
			const list = {
				id: +new Date(),
				name: moment().format("DD MMMM YYYY, HH:mm"),
				isBusy: true,
				isActive: true,
				items: [],
			};
			...
			try {
				...
				console.error("Error in Domains.vue >> ", error);
			} finally {
				list.isBusy = false;
